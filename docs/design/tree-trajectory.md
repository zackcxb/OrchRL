...
Due to the above reasons, we propose to store trajectories as prefix trees (aka tries) instead of sequences. A new branch is created when:

1. Two sampled trajectories start to diverge
...
Apart from the usual fields of a tree node, each node stores the following information:
```
role: str, "user" or "assistant". 
episode_ids: list[int], the episode ids that this node belongs to.
